//The integration tests will drop the database before running if the option dropdb is given.
...
module.exports = function(grunt) {
  ...
  grunt.registerTask('dropdb','drops bmm database.',function(){
    var done = this.async();
    var MongoClient = require('mongodb').MongoClient
    MongoClient.connect('mongodb://localhost/bmm', function(err, db) {
      if(err){
        throw new Error('Unable to connect to bmm.');
      }
      db.dropDatabase(function(err){
        db.close();
        if(err){
          throw new Error('Unable to drop bmm.');
        }
        done();
      });
    });
